(() => {
  ...
  const CONSENT_ERROR='Please check the notice before continuing or copying your order. يرجى تحديد مربع الإقرار قبل المتابعة أو نسخ الطلب.';
  function updateOrderConsent(){
    const consent=document.getElementById('order-consent');
    const accepted=Boolean(consent?.checked);
    consent?.setCustomValidity(accepted?'':CONSENT_ERROR);
    for(const id of ['send-order','copy-order']){const button=document.getElementById(id);if(button)button.disabled=!accepted;}
    const preview=document.getElementById('order-preview');if(preview){preview.hidden=!accepted;if(!accepted)preview.open=false;}
    const hint=document.getElementById('consent-help');if(hint)hint.hidden=accepted;
  ...
  function validateOrder(){
    const consent=document.getElementById('order-consent');
    if(!consent?.checked){
      consent?.setCustomValidity(CONSENT_ERROR);consent?.reportValidity();consent?.focus();
      const status=document.getElementById('form-status');if(status)status.textContent=CONSENT_ERROR;
      return false;
    }
    validatePhone();
    return Boolean(document.getElementById('order-form')?.reportValidity());
  }
  function updateMessage(){const box=document.getElementById('message-preview');if(box)box.value=document.getElementById('order-consent')?.checked?message():'';}
